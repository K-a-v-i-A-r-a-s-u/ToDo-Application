...
def login(request):
    if request.method == "POST":
        ...
        #     try:
        #         # user = request.user
        #         user_m=User.objects.get(username=user)
        #         data = Task.objects.filter(member=user_m)
        #         serializer = addtaskserializer(instance=data, many=True)
        #         response = {"msg": "Your tasks", "tasks": serializer.data}
        #         return Response(data=response)
...
def addtask(request):
    if request.method=="POST":
        ...
        # if serializer.is_valid():
        #     serializer.save()
        #     response={"msg":"task successfully added","data":serializer.data}
        #     return Response(data=response) 
        # else:
    ...
    return Response({"msg":"sdugguerhiughe"})
